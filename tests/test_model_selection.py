...
class TestChromaDBManagerModelSupport:
    ...
    def test_collection_name_generation(self, mock_client):
        ...
        # Test BGE model
        name = manager._generate_collection_name("BAAI/bge-large-en-v1.5", "subtitle_embeddings")
        assert name == "subtitle_embeddings_bge_large"

        # Test Gemma model
        name = manager._generate_collection_name("google/embeddinggemma-300m", "subtitle_embeddings")
        assert name == "subtitle_embeddings_gemma_300m"

        # Test unknown model
        name = manager._generate_collection_name("unknown/model", "subtitle_embeddings")
        assert name == "subtitle_embeddings_unknown"
    ...
    def test_get_collection_for_model(self, mock_client):
        ...
        expected_name = "subtitle_embeddings_bge_large"
    ...
    def test_list_collections_by_model(self, mock_client):
        ...
        mock_collections = [
            Mock(name="subtitle_embeddings_bge_large", metadata={"model_name": "BAAI/bge-large-en-v1.5"}),
            Mock(name="subtitle_embeddings_gemma_300m", metadata={"model_name": "google/embeddinggemma-300m"}),
        ...
        assert collections_by_model["BAAI/bge-large-en-v1.5"] == "subtitle_embeddings_bge_large"
        assert "google/embeddinggemma-300m" in collections_by_model
        assert collections_by_model["google/embeddinggemma-300m"] == "subtitle_embeddings_gemma_300m"
...
class TestQueryEngineModelSupport:
    ...
    def test_query_engine_model_initialization(self, mock_similarity_search):
        ...
        mock_similarity_search.assert_called_once_with(model_name="BAAI/bge-large-en-v1.5")
    ...
    def test_query_cache_with_model(self):
        ...
        cache_key2 = engine._get_cache_key("test query", 10, None, filters, "test/model")
        assert cache_key == cache_key2

        # Different model should produce different key
        cache_key3 = engine._get_cache_key("test query", 10, None, filters, "different/model")
...
class TestSimilaritySearchModelSupport:
    ...
    def test_similarity_search_model_initialization(self, mock_embedder, mock_chroma_manager):
    ...
    @patch('src.vector_store.chroma_manager.ChromaDBManager')
    @patch('src.embeddings.embedder.Embedder')
    def test_similarity_search_set_model(self, mock_embedder_class, mock_chroma_manager_class):
