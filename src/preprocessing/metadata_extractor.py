...
class MetadataExtractor:
    ...
    def extract_from_filename(self, file_path: Path) -> SourceMetadata:
        ...
        if not source_id:
            ...
            if len(parts_after_date) > 0:
                ...
                if potential_id and len(potential_id) >= 5:  # At least 5 chars
                    source_id = (
                        potential_id[:11] if len(potential_id) >= 11 else potential_id
                    )
                else:
            ...
            if parts_after_date:
                ...
                for part in parts_after_date:
                    ...
                    if len(parts) > 2:
                        ...
                        raise ValueError(
                            f"Cannot find source ID position in filename: {full_filename}"
                        )
            else:
                raise ValueError(
                    f"Cannot find source ID position in filename: {full_filename}"
        ...
        if not title:
            ...
            title_parts = [
                p for p in title_parts if p != date_match.group() and p != source_id
            ]
            title = " ".join(title_parts).strip()
            if not title:
                title = (
                    filename.replace(date_match.group(), "")
                    .replace(source_id, "")
                    .strip("_")
                )
