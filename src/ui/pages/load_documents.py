...
def validate_document_file(file) -> Tuple[bool, str]:
    ...
    if file_ext not in SUPPORTED_EXTENSIONS:
        ...
        return (
            False,
            f"Invalid file type: {file.name}. Supported formats: {supported_str}",
        )
    ...
    if file.size > max_size:
        return (
            False,
            f"File too large: {file.name} ({file.size / 1024 / 1024:.1f}MB). Max size is 10MB.",
        )
...
def process_files(
    ...
    try:
        ...
        embedding_pipeline = EmbeddingPipeline(
            model_name=model_name, batch_size=batch_size, enable_optimizations=True
        )
            ...
            for i, file_path in enumerate(file_paths):
                try:
                    ...
                    results["errors"].append(
                        f"Preprocessing error for {file_path.name}: {str(e)}"
                    )

                progress = (i + 1) / len(file_paths)
                preprocess_progress.progress(
                    progress, text=f"Parsed {i + 1}/{len(file_paths)} files"
                )
        ...
        with progress_container:
            ...
            for i, processed_document in enumerate(processed_documents):
                try:
                    ...
                    embeddings, metadata, actual_model = (
                        embedding_pipeline.generate_embeddings(
                            processed_document, show_progress=False
                        )
                    )
                    ...
                    # Update progress
                    progress = (
                        chunks_processed / total_chunks if total_chunks > 0 else 0
                    )
                    speed = (
                        len(processed_document.chunks) / embed_time
                        if embed_time > 0
                        else 0
                    )
        ...
        with progress_container:
            ...
            for i, item in enumerate(all_embeddings):
                ...
                index_progress.progress(
                    progress, text=f"Indexed {i + 1}/{len(all_embeddings)} documents"
                )
...
def render_load_documents_page() -> None:
    ...
    st.markdown("Import documents and generate embeddings for semantic search.")
