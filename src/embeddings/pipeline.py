...
class EmbeddingPipeline:
    ...
    def __init__(
        ...
        self.model_loader = get_model_loader(model_name=model_name, device=device)
    ...
    def generate_embeddings(
        self, processed_document: ProcessedDocument, show_progress: bool = True
        ...
        embeddings, metadata = self.batch_processor.process_chunks(
            processed_document.chunks, show_progress=show_progress
        )
    ...
    def generate_embeddings_batch(
        self, processed_documents: List[ProcessedDocument], show_progress: bool = True
        ...
        for i, processed_document in enumerate(processed_documents):
            ...
            embeddings, metadata = self.generate_embeddings(
                processed_document, show_progress=show_progress
            )
    ...
    def load_embeddings(
        self, input_path: Path, format: str = "numpy"
    ...
    def process_and_save(
        ...
        output_file = (
            output_dir
            / f"{processed_document.metadata.source_id}_embeddings.{save_format if save_format == 'json' else 'npz'}"
        )
    ...
    def generate_embeddings_with_checkpointing(
        ...
        """
        checkpoint_dir = checkpoint_dir or Path(
            getattr(self.config, "CHECKPOINT_DIR", "./data/checkpoints")
        )
        checkpoint_interval = checkpoint_interval or self.config.CHECKPOINT_INTERVAL

        checkpoint_dir.mkdir(parents=True, exist_ok=True)
        checkpoint_file = (
            checkpoint_dir / f"{processed_document.metadata.source_id}_checkpoint.pkl"
        )
            ...
            try:
                ...
                # Validate checkpoint model compatibility
                    ...
                    # Also validate embedding dimensions
                        ...
                        checkpoint_dim = (
                            len(checkpoint_embeddings[0])
                            if isinstance(checkpoint_embeddings[0], (list, np.ndarray))
                            else 0
                        )
                        expected_dim = self.embedder.get_embedding_dimension()
                        if checkpoint_dim != expected_dim:
                            ...
                            self.logger.info(
                                f"Resuming from chunk {start_idx}/{len(processed_document.chunks)}"
                            )
                    else:
                        start_idx = checkpoint_processed_count
                        accumulated_embeddings = checkpoint_embeddings
        ...
        if start_idx < len(processed_document.chunks):
            ...
            remaining_embeddings, remaining_metadata = (
                self.batch_processor.process_chunks(
                    remaining_chunks, show_progress=show_progress
                )
            )
            ...
            if accumulated_embeddings:
                embeddings = np.vstack(
                    [np.array(accumulated_embeddings), remaining_embeddings]
                )
            ...
            embeddings = (
                np.array(accumulated_embeddings)
                if accumulated_embeddings
                else np.array([])
            )
        ...
        if len(embeddings) != len(processed_document.chunks):
            ...
            if checkpoint_file.exists():
                checkpoint_file.unlink()
                self.logger.info(
                    "Cleared corrupted checkpoint, retrying from beginning"
                )
            raise ValueError(
    ...
    def clear_checkpoint(
        self, source_id: str, checkpoint_dir: Optional[Path] = None
    ) -> None:
        """
